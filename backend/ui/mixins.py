...
class WorkflowActionMixin:
    ...
    def _parse_date(self, name, required=False):
    ...
    def _parse_datetime(self, name, required=False):
        raw = self._get_value(name, required=required)
        if not raw:
            return None
    ...
    def _parse_list(self, name, *, required=False, label=None):
    ...
    def _resolve_assignees(self, field_name="assignees", alt_field="assignee_id"):
